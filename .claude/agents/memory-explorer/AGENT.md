...
description: Use PROACTIVELY before answering any question about why this codebase is the way it is — invariants, rationale, gotchas, dead ends, "what breaks if I change X", "has this been tried". Consults the repo-resident memory store first, re-verifies what has gone stale, and stages what it learned for curation. Do NOT use for locating code ("where is X", "what calls Z") — grep, the LSP and serena answer those in seconds, and this agent deliberately does not store them.
model: sonnet
tools:
- Read
- Grep
