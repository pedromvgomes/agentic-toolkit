...
Re-checked the target note's claim that `legacyTopLevelKeys` (`source/toolkit/internal/stack/parser.go:372`)
...
unified stack manifest (v2)") had `source/toolkit/internal/config/types.go:19`:
...
in `source/toolkit/internal/definitions/types.go:76-81` today (`claude`, `cursor`, `copilot`, `opencode`,
...
`legacyTopLevelKeys` (`source/toolkit/internal/stack/parser.go:372`) is a pure string-match guard
...
(`source/toolkit/internal/stack/tests/parser_test.go:85-96`) only exercises `source`+`presets` together; no
...
per the old `source/toolkit/internal/config/types.go` doc comment "Slice-1 scope: source, platforms,
