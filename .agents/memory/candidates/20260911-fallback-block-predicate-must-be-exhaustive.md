...
`Run`'s fallback trigger (`source/toolkit/internal/reviewrun/run.go`) is `onlyBlocked(reports)`: it walks
...
`source/toolkit/internal/cli/codereview_pr.go` withholds the PR post — even though the actual reason the
...
This was not caught by the original test suite — `source/toolkit/internal/reviewrun/fallback_test.go` only
