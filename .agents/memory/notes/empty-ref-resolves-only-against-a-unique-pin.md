...
`source/toolkit/internal/sourcestore/git.go:68-70`), so the literal key never matches. Covered by
`TestFrozenProvider_EmptyConfigRef_FallsBackToUniquePinForURL`
(`source/toolkit/internal/sourcestore/tests/frozen_provider_test.go:36`).
...
sources on `(URL, Ref)` (`source/toolkit/internal/resolver/sources.go:9`), so `agtk lock` writes both rows
