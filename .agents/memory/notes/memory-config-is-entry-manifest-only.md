...
`internal/resolver/resolver.go:191`. The entry stack is the one built with `Identifier: ""`
...
exists on `stack.Stack` (`internal/stack/types.go:63`) and the schema is strict, so nothing
rejects it — and the only trace is one informational diagnostic, `DiagIgnoredMemoryConfig`
(declared `internal/resolver/types.go:142-146`, emitted `resolver.go:193`). It is never a hard
...
`(*Stack).MemoryRoot()` (`internal/stack/types.go:79`) has no notion of entry-vs-extended; it
