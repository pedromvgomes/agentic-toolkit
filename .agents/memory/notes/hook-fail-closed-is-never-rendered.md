  ...
  - path: internal/adapters/claude/*.go
    matches:
      ...
      - path: internal/adapters/claude/instructions.go
        blob: 39b42509cf63
      - path: internal/adapters/claude/mcp.go
        blob: 69ecb724a03f
...
`Hook.FailClosed` is declared at `internal/definitions/types.go:275`
...
Nothing renders it. `collectHooks` (`internal/adapters/claude/settings.go:202`) is the only
...
otherwise appears only in the schemagen example (`tools/schemagen/main.go:103`), the generated
docs, and a test fixture. `internal/adapters/` holds exactly one adapter (`claude`), so there
