...
`memory.New(projectRoot, root)` (`internal/memory/store.go:31-40`) joins a relative root onto
`projectRoot` and takes an absolute root as-is. It checks nothing.

The confinement rule lives in `ValidateRoot` (`internal/memory/store.go:46-57`), a free
...
Exactly one caller pairs them, in the CLI: `internal/cli/memory.go:134` runs
...
`ensureGitignore` (`internal/memory/hits.go:30`) creates the root as a side effect of a plain
