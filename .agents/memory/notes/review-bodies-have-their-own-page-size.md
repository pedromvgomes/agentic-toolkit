...
anchors:
  - path: source/toolkit/internal/githubapp/replies.go
    blob: fe91de20c87e
  - path: source/toolkit/internal/githubapp/graphql.go
    blob: 5ab99057e716
...
`source/toolkit/internal/githubapp` has two page-size constants and they are not interchangeable.
`pageSize = 25` (`source/toolkit/internal/githubapp/graphql.go:99`) is for reads that carry no body — a commit
oid, a thread's first comment. `reviewsPerPage = 10` (`source/toolkit/internal/githubapp/replies.go:206`)
...
The bound that makes this matter is `maxBody = 1 << 20` (`source/toolkit/internal/githubapp/client.go:31`),
...
`ReadSubmittedReviews` (`source/toolkit/internal/githubapp/replies.go:231`) is the **only** reader of review
