  ...
  - path: internal/resolver/*.go
    matches:
      ...
      - path: internal/resolver/types.go
        blob: f324cf4d5af4
  - path: internal/definitions/walk.go
    blob: a7f4470eb69f
confidence: verified
---
...
1. A stack manifest entry. `loadStack` iterates `st.EntriesFor(cat)` for each category
   (`internal/resolver/resolver.go:202-203`) and populates `s.overlay` only from what that
...
2. A `requires:` on an already-resolved definition. `pullOne` iterates
   `w.Definition.GetCommon().Requires` (`internal/resolver/requires.go:99`) and resolves each
...
Do not read `internal/definitions/walk.go` as evidence against this — and do not read it as
dead code either. `WalkCatalog` (`walk.go:31`) and `isEntryPoint` really do enumerate
`definitions/`, and they have callers:
`internal/definitions/tests/catalog_refs_test.go:19,47,130` and
`internal/definitions/tests/catalog_test.go:23`. `catalog_refs_test.go` walks the *real*
...
path. Both walking call sites are under `internal/definitions/tests/`; no non-test file calls
`WalkCatalog`.

`internal/resolver/*.go` is a glob because the claim quantifies — *no* resolution path
