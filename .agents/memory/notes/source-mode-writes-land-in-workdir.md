...
the write root diverge: `stackDir` returns `SourceDir` while `lockfilePath` returns
`WorkDir` (`internal/cli/paths.go`), and the memory store follows the same split through
`memoryProjectRoot` (`internal/cli/memory.go`).
...
the source's for the same reason: where a repo commits its notes is not a shared tree's
decision. See [[nonzero-exit-needs-a-sentinel-in-execute]] for the other cross-command
