...
anchors:
  - path: internal/completioninstall/install.go
    blob: 71b67e0cd008
...
(`internal/completioninstall/install.go:242`) runs `exec.Command(exe, "completion", shell)`
...
1. **Ordering.** `runUpdate` calls `installer.Install(info.Latest)` first
   (`internal/cli/update.go:113`) and `completioninstall.Install` only afterwards (`:124`).
...
2. **Non-fatal by construction.** The refresh's error is printed to stderr as a hint and
   discarded (`internal/cli/update.go:124-127`); `runUpdate` still returns nil. The binary
