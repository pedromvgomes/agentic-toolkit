...
**Panel**, **Handoff**, **Task**, and more — read [CONTEXT.md](CONTEXT.md) before writing about
any of those concepts. Terms there are precise and words listed under `_Avoid_` are deliberately
banned; using a wrong synonym (e.g. "preset" for **Stack**, "gate" for **Predecessor**) reads as
a misunderstanding, not a style choice.
...
- `internal/` — CLI implementation (private packages): `resolver`, `lockfile`, `stack`,
  ...
  `adapters` (per-platform render targets)
...
`internal/definitions` validates against it at render time.
