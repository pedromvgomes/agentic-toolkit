...
var glossaryScope = []string{
	...
	"source/toolkit/internal/review",
...
func TestTheReviewPackagesSayCandidateFinding(t *testing.T) {
	...
	err := filepath.Walk(filepath.Join(repo, "source/toolkit/internal/reviewrun"), func(path string, info os.FileInfo, err error) error {
		if err != nil || info.IsDir() || !strings.HasSuffix(path, ".go") {
			return err
		}
		body, err := os.ReadFile(path) // #nosec G304 -- a .go file inside this repository
