// Package schemadoc renders the human-facing schema docs from the Go struct
// definitions in internal/definitions, internal/stack and internal/lockfile.
// The structs are the canonical schema; this package keeps the documentation in
// lockstep with them.
//
// Run through the generator at tools/schemagen:
//
//	make generate
//
// or directly, from the module directory:
//
//	go -C source/toolkit run ./tools/schemagen
//
// The module is source/toolkit, so a bare `go generate ./...` from the repo root
// finds no module at all.
//
// Output:
//   - definitions/SCHEMA.md         — toolkit-side definitions schema
//   - definitions/CONFIG-SCHEMA.md  — consumer-side stack + lockfile schema
//
// The entrypoint is a separate package holding nothing but func main, so that
// everything here is reachable from a test. A func main is not.
package schemadoc
...
var categories = []categoryDoc{
	...
	},
	{
		...
		Title:       "Settings",
		PathPattern: "definitions/settings/<name>.yaml",
		FileShape:   "YAML manifest (no body)",
		Intro:       "A setting carries an arbitrary fragment that adapters merge into the platform's settings file (e.g. Claude Code's `settings.json`). The fragment is intentionally untyped: agtk does not validate the platform's settings vocabulary — adapters do, at render time. Top-level keys in `value` map 1:1 to top-level keys in the rendered settings file. Override semantics are last-preset-wins on the whole definition (not on individual `value` keys); adapters track agtk-managed top-level keys via a `_meta.agtk.managed` marker so user-edited keys are preserved across re-renders.",
		Sample:      &defs.Setting{},
		Example:     "name: deny-dangerous-shell\ndescription: Block destructive shell commands.\nvalue:\n  permissions:\n    deny:\n      - \"Bash(rm -rf:*)\"\n      - \"Bash(sudo rm:*)\"\n  env:\n    AGTK_DEBUG: \"1\"\n",
		Notes:       "**Pass-through.** The `value` map is decoded as `map[string]any`; agtk performs no platform-specific validation. Adapters validate keys against the target platform's settings schema and skip-with-warn on unsupported keys.\n\n**Granularity.** A single Setting definition is the dedupe unit. To split ownership of multiple top-level keys across presets, use one Setting definition per concern.",
	},
}
...
// Generate writes both schema documents into the catalog of the repo the
...
func render() ([]byte, error) {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "<!-- DO NOT EDIT — generated by source/toolkit/tools/schemagen from source/toolkit/internal/definitions struct definitions. Run `make generate` to refresh. -->")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "Every asset in `definitions/` parses into one of eight category structs. The struct definitions in `source/toolkit/internal/definitions/types.go` are the canonical schema; this document is regenerated from them.")
	fmt.Fprintln(&b)
...
func renderConfig() ([]byte, error) {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "<!-- DO NOT EDIT — generated by source/toolkit/tools/schemagen from source/toolkit/internal/stack and source/toolkit/internal/lockfile struct definitions. Run `make generate` to refresh. -->")
	fmt.Fprintln(&b)
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "Settings for the repo-resident memory store (`agtk memory ...`): where the store lives and which agent manages it.")
	fmt.Fprintln(&b)
