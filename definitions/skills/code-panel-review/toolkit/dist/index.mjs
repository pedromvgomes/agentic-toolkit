  ...
  "node_modules/vscode-jsonrpc/lib/node/main.js"(exports) {
    ...
    function generateRandomPipeName() {
      ...
      if (XDG_RUNTIME_DIR) {
        ...
        result = path5.join(os2.tmpdir(), `vscode-${randomSuffix}.sock`);
      }
...
var SKILL_ROOT = path.resolve(import.meta.dirname, "..", "..");
var PROMPTS_DIR = path.join(SKILL_ROOT, "prompts");
var CONFIG_FILE = ".code-panel-review.yaml";
var DEFAULTS = {
  security: { model: "claude-opus-4.6", prompt: "SECURITY-REVIEW.md" },
  performance: { model: "gpt-5.4", prompt: "PERFORMANCE-REVIEW.md" },
  resilience: { model: "claude-opus-4.6", prompt: "RESILIENCE-REVIEW.md" },
  bugs: { model: "gpt-5.4", prompt: "BUGS-REVIEW.md" },
  compliance: { model: "gpt-5.4", prompt: "COMPLIANCE-REVIEW.md" }
};
var DEFAULT_TIMEOUT_SECONDS = 600;
function loadConfig(repoRoot) {
  const yamlPath = path.join(repoRoot, CONFIG_FILE);
  let raw = {};
  if (fs.existsSync(yamlPath)) {
    const text = fs.readFileSync(yamlPath, "utf8");
    raw = import_yaml.default.parse(text) ?? {};
    warnUnknownKeys(raw, ["reviewers", "settings", "diff"]);
  }
  const settings = raw.settings ?? {};
  const reviewersRaw = raw.reviewers ?? {};
  const reviewers = Object.keys(DEFAULTS).map((name) => {
    const override = reviewersRaw[name] ?? {};
    const enabled = override.enabled === void 0 ? true : !!override.enabled;
    const model = override.model ?? DEFAULTS[name].model;
    const promptPath = resolvePromptPath(
      override.prompt,
      DEFAULTS[name].prompt,
      repoRoot
    );
    return { name, model, promptPath, enabled };
  });
  if (!reviewers.some((r) => r.enabled)) {
    throw new Error(
      "All reviewers are disabled in .code-panel-review.yaml \u2014 nothing to do."
    );
  }
  return {
    reviewers,
    timeoutSeconds: settings.timeout_seconds ?? DEFAULT_TIMEOUT_SECONDS,
    parallel: settings.parallel === void 0 ? true : !!settings.parallel
  };
}
function resolvePromptPath(override, defaultFile, repoRoot) {
  if (override) {
    if (path.isAbsolute(override)) {
      throw new Error(
        `Prompt override must be a repo-relative path (got absolute: ${override}). This guard prevents a hostile .code-panel-review.yaml from reading arbitrary local files via the reviewer's system prompt.`
      );
    }
    ...
    if (rel.startsWith("..") || path.isAbsolute(rel)) {
      throw new Error(
        `Prompt override must stay inside the repo (got: ${override}).`
      );
    }
    if (!fs.existsSync(resolved)) {
      throw new Error(`Prompt override not found: ${resolved}`);
    }
    return resolved;
  }
  return path.join(PROMPTS_DIR, defaultFile);
}
function warnUnknownKeys(obj, known) {
  for (const key of Object.keys(obj)) {
    if (!known.includes(key)) {
      process.stderr.write(
        `[panel] warning: unknown config key "${key}" \u2014 ignoring
`
      );
    }
  }
}
function resolveModelId(requested, available) {
  const req = requested.toLowerCase();
  const lower = available.map((m) => m.toLowerCase());
  const exact = available.find((m) => m === requested);
  if (exact) return exact;
  const ci = available.filter((_, i) => lower[i] === req);
  if (ci.length === 1) return ci[0];
  if (ci.length > 1) {
    throw new Error(
      `Ambiguous model "${requested}" \u2014 matches: ${ci.join(", ")}`
    );
  }
  const prefix = available.filter((_, i) => lower[i].startsWith(req));
  if (prefix.length === 1) return prefix[0];
  if (prefix.length > 1) {
    throw new Error(
      `Ambiguous model "${requested}" (prefix match) \u2014 candidates: ${prefix.join(", ")}. Pin the exact id in .code-panel-review.yaml.`
    );
  }
  const substr = available.filter((_, i) => lower[i].includes(req));
  if (substr.length === 1) return substr[0];
  if (substr.length > 1) {
    throw new Error(
      `Ambiguous model "${requested}" (substring match) \u2014 candidates: ${substr.join(", ")}. Pin the exact id in .code-panel-review.yaml.`
    );
  }
  throw new Error(
    `Model "${requested}" not available. Known models: ${available.join(", ") || "(none)"}`
  );
}
...
function buildManifest(repoRoot, scope) {
  const matchesAnyExclude = (p) => scope.excludes.some(
    (pattern) => path2.matchesGlob(p, pattern) || // Bare patterns (no "/") should match basenames anywhere in the tree.
    !pattern.includes("/") && path2.matchesGlob(path2.basename(p), pattern)
  );
  const headRef = git(repoRoot, ["rev-parse", "--abbrev-ref", "HEAD"]).trim();
  const baseRef = scope.includeCommittedUnpushed ? detectBase(repoRoot) : null;
  const files = /* @__PURE__ */ new Map();
  if (scope.includeCommittedUnpushed && baseRef) {
    collectDiff(repoRoot, ["diff", "--unified=0", `${baseRef}...HEAD`], files);
  }
  if (scope.includeStaged) {
    collectDiff(repoRoot, ["diff", "--unified=0", "--cached"], files);
  }
  if (scope.includeUnstaged) {
    collectDiff(repoRoot, ["diff", "--unified=0"], files);
  }
  if (scope.includeUntracked) {
    collectUntracked(repoRoot, files, matchesAnyExclude);
  }
  const changedFiles = Array.from(files.entries()).filter(([p]) => !matchesAnyExclude(p)).map(([path5, ranges]) => ({
    path: path5,
    changed_lines: mergeRanges(
      Array.from(ranges).map((s) => {
  ...
  return {
    ...
    head_ref: headRef,
    scope: {
      committed_unpushed: scope.includeCommittedUnpushed,
      staged: scope.includeStaged,
      unstaged: scope.includeUnstaged,
      untracked: scope.includeUntracked
    },
    files: changedFiles
  };
}
function git(repoRoot, args) {
  try {
    ...
    if (e && (e.killed || e.code === "ETIMEDOUT" || e.signal === "SIGKILL")) {
      throw new Error(
...
function detectBase(repoRoot) {
  try {
    ...
    try {
      ...
      throw new Error(
        "No upstream tracking branch and no origin/HEAD \u2014 set an upstream with `git push -u origin <branch>` or specify the committed_unpushed scope explicitly."
      );
    }
  }
}
function collectDiff(repoRoot, args, acc) {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (line.startsWith("+++ ")) {
      ...
      if (current && !acc.has(current)) acc.set(current, /* @__PURE__ */ new Set());
      continue;
    }
    if (line.startsWith("@@") && current) {
      if (isDeletion) {
        ...
        if (!m2) continue;
        const start2 = parseInt(m2[1], 10);
        const count2 = m2[2] ? parseInt(m2[2], 10) : 1;
        if (count2 === 0) continue;
        const end2 = start2 + count2 - 1;
        acc.get(current).add(`${Math.max(1, start2)}-${Math.max(1, end2)}`);
        continue;
      }
      ...
      if (count === 0) {
        ...
        acc.get(current).add(`${anchor}-${anchor}`);
        continue;
      }
      const end = start + count - 1;
      acc.get(current).add(`${start}-${end}`);
    }
  }
}
function collectUntracked(repoRoot, acc, isExcluded) {
  ...
  if (!raw) return;
  for (const path5 of raw.split("\n")) {
    if (!path5) continue;
    if (isExcluded(path5)) continue;
    const lineCount = safeLineCount(repoRoot, path5);
    const set = acc.get(path5) ?? /* @__PURE__ */ new Set();
    set.add(`1-${Math.max(1, lineCount)}`);
    acc.set(path5, set);
  }
}
var LINE_COUNT_MAX_BYTES = 2 * 1024 * 1024;
function safeLineCount(repoRoot, relPath) {
  try {
    const abs = path2.resolve(repoRoot, relPath);
    const st = fs2.statSync(abs);
    if (!st.isFile()) return 1;
    if (st.size === 0) return 1;
    if (st.size > LINE_COUNT_MAX_BYTES) {
      return Math.max(1, Math.min(Math.ceil(st.size / 64), 2e5));
    }
    return countNewlinesStreaming(abs);
  } catch {
    return 1;
  }
}
function countNewlinesStreaming(abs) {
  ...
  try {
    ...
    let lastByte = 0;
    while (true) {
      const read = fs2.readSync(fd, buf, 0, buf.length, null);
      if (read === 0) break;
...
// src/types.ts
var ReviewerName = external_exports.enum([
  "security",
  "performance",
  "resilience",
  "bugs",
...
var Confidence = external_exports.enum(["high", "medium", "low"]);
var Finding = external_exports.object({
  id: external_exports.string().min(1),
  file: external_exports.string().min(1),
  line_start: external_exports.number().int().min(1),
  line_end: external_exports.number().int().min(1),
  severity: Severity,
  confidence: Confidence,
  category: external_exports.string().min(1),
  title: external_exports.string().min(1),
  finding: external_exports.string().min(1),
...
// src/agent.ts
var EXCLUDED_TOOLS = ["shell", "write"];
async function runAgent(client, reviewer, manifest, availableModels, timeoutSeconds, allowedHosts) {
  const started = Date.now();
  const modelId = resolveModelId(reviewer.model, availableModels);
  const systemPrompt = fs3.readFileSync(reviewer.promptPath, "utf8");
  log(reviewer.name, `starting (model=${modelId})`);
  ...
  try {
    const result = await withTimeout(
      ...
      timeoutSeconds * 1e3,
      `reviewer "${reviewer.name}" timed out after ${timeoutSeconds}s`,
      async () => {
        ...
        if (s && typeof s.close === "function") {
          try {
            ...
            log(
              reviewer.name,
              `session close on timeout failed: ${e instanceof Error ? e.message : String(e)}`
            );
          }
        }
      }
    );
    log(reviewer.name, `finished in ${Date.now() - started}ms`);
    return { ...result, durationMs: Date.now() - started };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    log(reviewer.name, `failed: ${message}`);
    return {
...
async function runOnce(client, modelId, systemPrompt, manifest, reviewerName, allowedHosts, sessionRef, timeoutSeconds, repoRoot) {
  const manifestJson = JSON.stringify(
    { files: manifest.files, repo_root: manifest.repo_root },
    null,
    2
  );
  const userPrompt = buildUserPrompt(manifestJson, modelId);
  const firstRaw = await send(
    client,
    modelId,
    systemPrompt,
    userPrompt,
  ...
  const firstParsed = tryParse(firstRaw, reviewerName);
  if (firstParsed.ok) {
    return {
      reviewer: reviewerName,
  ...
  log(
    reviewerName,
    `schema validation failed on first attempt: ${firstParsed.error}. Retrying once.`
  );
  const retryPrompt = userPrompt + `

---

Your previous response failed schema validation with the error below. Return only a single JSON object conforming to the schema \u2014 no prose, no markdown fences.

Validation error: ${firstParsed.error}

Your previous response (first 2000 chars):
${firstRaw.slice(0, 2e3)}`;
  const secondRaw = await send(
    client,
    modelId,
    systemPrompt,
    retryPrompt,
  ...
  const secondParsed = tryParse(secondRaw, reviewerName);
  if (secondParsed.ok) {
    return {
      reviewer: reviewerName,
      output: secondParsed.value,
      rawOnFailure: null,
      modelId
    };
  }
  return {
    reviewer: reviewerName,
    output: null,
    rawOnFailure: secondRaw,
    modelId,
    error: `Schema validation failed twice: ${secondParsed.error}`
  };
}
function makePermissionHandler(allowedHosts, repoRoot) {
  const repoRootAbs = path3.resolve(repoRoot);
  return (request) => {
    if (request.kind === "read") {
      const targets = extractReadPaths(request);
      if (targets.length === 0) {
        return {
          kind: "denied-by-rules",
          message: "Read permission request missing a path \u2014 denied."
        };
      }
      for (const target of targets) {
        const abs = path3.resolve(repoRootAbs, target);
        const rel = path3.relative(repoRootAbs, abs);
        if (rel !== "" && (rel.startsWith("..") || path3.isAbsolute(rel))) {
          return {
            kind: "denied-by-rules",
            message: `Read path "${target}" escapes repo root \u2014 denied.`
          };
        }
      }
    ...
    if (request.kind === "url") {
      ...
      if (!parsed) {
        return {
          kind: "denied-by-rules",
          message: `URL "${url}" is unparseable \u2014 denied.`
        };
      }
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        return {
          kind: "denied-by-rules",
          message: `URL scheme "${parsed.protocol}" is not allowed \u2014 only http/https.`
        };
      }
      if (parsed.username || parsed.password) {
        return {
          kind: "denied-by-rules",
          message: "URLs containing userinfo are denied."
        };
      }
      const host = parsed.hostname.toLowerCase();
      if (host && hostMatches(host, allowedHosts)) {
        return { kind: "approved" };
      }
      return {
        kind: "denied-by-rules",
        message: `URL host "${host || "(empty)"}" is not in allowed-hosts.`
      };
    }
    return {
      kind: "denied-by-rules",
      message: `Panel reviewers allow only read/url; denied "${request.kind}" request.`
    };
  };
}
function extractReadPaths(request) {
  const out = [];
...
function wireProgressEvents(session, emit) {
...
function summarizeArgs(_toolName, args) {
  const preferred = [
    "path",
    "file_path",
    "filePath",
  ...
  for (const key of preferred) {
    ...
    if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string") {
      return truncate(`${v[0]}${v.length > 1 ? ` (+${v.length - 1})` : ""}`, 80);
    }
  }
  return "";
}
...
async function send(client, modelId, systemPrompt, userPrompt, reviewerName, allowedHosts, sessionRef, timeoutSeconds, repoRoot) {
  const session = await client.createSession({
    model: modelId,
    streaming: false,
    systemMessage: { mode: "replace", content: systemPrompt },
    excludedTools: EXCLUDED_TOOLS,
    onPermissionRequest: makePermissionHandler(allowedHosts, repoRoot)
  });
  sessionRef.current = session;
  wireProgressEvents(session, (m) => log(reviewerName, m));
  try {
    ...
    if (typeof content !== "string") {
      throw new Error("SDK returned no string content");
    }
    return content;
  } finally {
...
function buildUserPrompt(manifestJson, modelId) {
  return [
    `You are running under model id: ${modelId}.`,
    `Set the "model" field in your output to exactly "${modelId}".`,
    "",
    "Below is the changed-files manifest for this review. Use the filesystem tool",
    "to read the full contents of each listed file before forming findings.",
    "The `changed_lines` ranges tell you where the change is; evaluate the change's",
    "impact on the surrounding code, not just those lines.",
    "",
    "```json",
    manifestJson,
    "```",
    "",
    "Return ONLY the single JSON object described in your system prompt. No prose."
  ].join("\n");
}
function tryParse(raw, reviewerName) {
  ...
  const result = ReviewerOutput.safeParse(parsed);
  if (!result.success) {
    return {
      ok: false,
  ...
  if (result.data.reviewer !== reviewerName) {
    return {
      ok: false,
      error: `reviewer field was "${result.data.reviewer}", expected "${reviewerName}"`
    };
  }
...
function withTimeout(p, ms, message, onTimeout) {
  return new Promise((resolve, reject) => {
    let settled = false;
...
var logBuffer = [];
var flushTimer = null;
function log(reviewer, msg) {
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  logBuffer.push(`[panel ${ts}] ${reviewer}: ${msg}`);
  if (!flushTimer) {
...
async function main() {
  ...
  if (!Number.isFinite(nodeMajor) || nodeMajor < MIN_NODE_MAJOR) {
    process.stderr.write(
      `[panel] error: Node ${MIN_NODE_MAJOR}+ required, running on v${process.versions.node}.
...
`
  ...
  if (!args.scope.includeCommittedUnpushed && !args.scope.includeStaged && !args.scope.includeUnstaged && !args.scope.includeUntracked) {
    process.stderr.write(
      "[panel] error: no scope flags provided \u2014 pass at least one of --include-committed-unpushed, --include-staged, --include-unstaged, --include-untracked\n"
    );
    return 2;
  }
  const manifest = buildManifest(args.repo, args.scope);
  if (manifest.files.length === 0) {
    process.stderr.write("[panel] nothing to review \u2014 manifest is empty\n");
    return 3;
  }
  const config = loadConfig(args.repo);
  const enabled = config.reviewers.filter((r) => r.enabled);
  const allowedHosts = loadAllowedHosts(
    args.allowedHostsFile,
    args.allowedHostsInline
  );
  const client = new CopilotClient();
  let availableModels = [];
  try {
    ...
    try {
      availableModels = await listModels(client);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      process.stderr.write(
        `[panel] failed to list models from Copilot SDK: ${msg}
       Run \`copilot auth login\` if you haven't authenticated, and make sure your subscription is active.
`
      );
      return 4;
    }
    const outDir = fs4.mkdtempSync(
      path4.join(os.tmpdir(), "panel-review-")
    );
    writeManifest(outDir, manifest);
    const tasks = enabled.map(
      (r) => () => runAgent(client, r, manifest, availableModels, config.timeoutSeconds, allowedHosts)
    );
    const results = config.parallel ? await Promise.all(tasks.map((t) => t())) : await runSerially(tasks);
    for (const res of results) {
      if (res.output) {
        fs4.writeFileSync(
          path4.join(outDir, `${res.reviewer}.json`),
          JSON.stringify(res.output, null, 2)
        );
      } else {
        fs4.writeFileSync(
          path4.join(outDir, `${res.reviewer}.error.json`),
          JSON.stringify(
            {
              reviewer: res.reviewer,
              model: res.modelId,
              error: res.error ?? "unknown",
              raw: res.rawOnFailure,
    ...
    process.stdout.write(`PANEL_OUTPUT_DIR=${outDir}
`);
    return 0;
  } finally {
...
async function listModels(client) {
  ...
  const candidates = ["listModels", "availableModels", "getModels"];
  for (const name of candidates) {
    if (typeof anyClient[name] === "function") {
      const result = await anyClient[name]();
      if (Array.isArray(result)) {
        return result.map(
          (m) => typeof m === "string" ? m : m?.id ?? m?.name ?? String(m)
        );
      }
    }
  }
  throw new Error(
    "CopilotClient does not expose a listModels-compatible method (tried: listModels, availableModels, getModels). SDK API may have changed \u2014 update toolkit/src/index.ts."
  );
}
function loadAllowedHosts(file, inline) {
  ...
  if (file) {
    if (!fs4.existsSync(file)) {
      throw new Error(`--allowed-hosts-file not found: ${file}`);
    }
...
function writeManifest(outDir, manifest) {
  fs4.writeFileSync(
    path4.join(outDir, "manifest.json"),
...
function parseArgs(argv) {
  const scope = {
    includeCommittedUnpushed: false,
    includeStaged: false,
  ...
  let allowedHostsFile = null;
  const allowedHostsInline = [];
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    switch (a) {
      case "--repo":
        repo = path4.resolve(argv[++i]);
        break;
      case "--include-committed-unpushed":
        scope.includeCommittedUnpushed = true;
        break;
      case "--include-staged":
        scope.includeStaged = true;
        break;
      case "--include-unstaged":
        scope.includeUnstaged = true;
        break;
      case "--include-untracked":
        scope.includeUntracked = true;
        break;
      case "--exclude": {
        const pat = argv[++i];
        if (!pat) {
          process.stderr.write("[panel] --exclude requires a glob pattern\n");
          process.exit(2);
        }
        scope.excludes.push(pat);
        break;
      }
      case "--allowed-hosts-file": {
        const p = argv[++i];
        if (!p) {
          process.stderr.write("[panel] --allowed-hosts-file requires a path\n");
          process.exit(2);
        }
        allowedHostsFile = path4.resolve(p);
        break;
      }
      case "--allowed-host": {
        const h = argv[++i];
        if (!h) {
          process.stderr.write("[panel] --allowed-host requires a hostname\n");
          process.exit(2);
        }
        allowedHostsInline.push(h);
        break;
      }
      case "-h":
      case "--help":
        ...
        process.exit(0);
      default:
        process.stderr.write(`[panel] unknown arg: ${a}
`);
        process.exit(2);
    }
  }
  return { repo, scope, allowedHostsFile, allowedHostsInline };
}
var HELP = `code-panel-review toolkit

Usage:
  node dist/index.mjs --repo <path> [scope flags...] [filters...] [hosts...]

Scope flags (pass at least one):
  --include-committed-unpushed   Commits on HEAD not yet on upstream
  --include-staged               Staged working-tree changes
  --include-unstaged             Unstaged working-tree changes
  --include-untracked            New files not covered by .gitignore

Filters (optional, repeatable):
  --exclude <glob>               Drop matching paths from the manifest
                                  Supports *, **, ?. Patterns without "/"
                                  match the basename; with "/" match the full
                                  repo-relative path.

Web hosts (at least one of --allowed-hosts-file / --allowed-host is recommended;
url requests to hosts outside the allowlist are denied at runtime):
  --allowed-hosts-file <path>    File with one hostname per line, # for comments
  --allowed-host <hostname>      Repeatable; add a single hostname inline

Output:
  Prints PANEL_OUTPUT_DIR=<path> on stdout when complete.
  Each enabled reviewer produces <reviewer>.json in that directory
  (or <reviewer>.error.json if schema validation fails twice).
`;
main().then(
  (code) => {
    flushLog();
  ...
  (err) => {
    flushLog();
    process.stderr.write(`[panel] fatal: ${err?.stack ?? err}
`);
    process.exit(1);
  }
);
