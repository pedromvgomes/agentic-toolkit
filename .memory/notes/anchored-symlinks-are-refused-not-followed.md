...
anchors:
  - path: source/toolkit/internal/memory/lint.go
    blob: 55ed2c6534eb
  - path: source/toolkit/internal/memory/anchor.go
    blob: a8d7491d9e82
  - path: source/toolkit/internal/memory/audit.go
    blob: b7fa6df6e433
  - path: source/toolkit/internal/memory/blob.go
    blob: 5a7a3778d65a
confidence: verified
---

`ValidateAnchorPath` (`source/toolkit/internal/memory/lint.go:26`) is purely lexical — `**`, `filepath.IsAbs`,
...
**It is not on its own.** `contained` (`source/toolkit/internal/memory/anchor.go:148`) resolves the whole path
...
- audit: a leaf symlink is refused outright as `DriftInvalid` "is a symlink, which is not
  anchorable" (`source/toolkit/internal/memory/audit.go:96-104`), and containment is checked at `:105-111`
  *before* `HashFile` at `:112`, because `HashFile` is an `os.ReadFile`
  (`source/toolkit/internal/memory/blob.go:31-32`) that resolves every component.
...
- **`Lstat` on the final component is not enough** (`anchor.go:143-147`). The kernel resolves
  every directory above it, so `source/toolkit/internal/x -> ~/.ssh` with the anchor `source/toolkit/internal/x/id_rsa` names
...
neighbours in `source/toolkit/internal/memory/tests/anchor_test.go:331`.
