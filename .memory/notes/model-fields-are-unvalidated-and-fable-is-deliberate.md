...
`Agent.Model` (`source/toolkit/internal/definitions/types.go:181`) is a plain `string` whose `agtkdoc` tag
names the shorthands. Nothing checks a value against them: the renderer passes `a.Model`
straight into the frontmatter (`source/toolkit/internal/adapters/claude/files.go:225`), and no non-test file
