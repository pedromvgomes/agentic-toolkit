...
`sync.go:73` construct the live one. `fetch.go:42`, `plan.go:54`, `render.go:71`,
`status.go:82` and `sync.go:93` construct the frozen one, so a CI run that has a lockfile can
