...
38 notes. Read one with `agtk memory show <name>`.
...
## two-committed-trees-live-under-dot-agents  (gotcha, verified)
A blanket /.agents/ gitignore rule would stop tracking .agents/code-review/manifest.yaml as well as the memory store, and the rule's own comment names only the store.
